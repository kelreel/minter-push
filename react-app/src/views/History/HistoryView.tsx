...
const HistoryView: React.FC = observer(() => {
  ...
  const getList = () => {
    return state.history?.map(item => {
      if (item.type === historyEntryType.push) {
        return <div key={item.link} className="item wallet">
          <div className="row">
            <div className="name">{item.link}</div>
            <div className="row">
              {item.status && <div className="status">
                  <Tag color={statusColor(item.status.status)}>
                    {item.status.status.toUpperCase()}
                  </Tag>
              </div>}
              <Button size="small"
                      onClick={() => window.open(`${config.domain}${item.link}`, '_blank')}>Open</Button>
            </div>
          </div>
          {item.password && <div className="label">Password: {item.password}</div>}
          {item.date && <div className="label">{(new Date(item.date)).toLocaleString()}</div>}
        </div>
      } else {
        return <div key={item.link} className="item campaign">
          <div className="row">
            <div className="name"><Icon style={{marginRight: '5px'}} type="share-alt"/>Campaign: {item.link}</div>
            <Button size="small"
                    onClick={() => window.open(`${config.domain}multi/${item.link}`, '_blank')}>Open</Button>
          </div>
          {item.password && <div className="label">Password: {item.password}</div>}
        </div>
      }
  ...
  return (
    <Content className="history-view">
      <Title level={3} style={{marginBottom: "35px"}} className="animated fadeIn">
        History
      </Title>
      {!state.history && <Button onClick={() => history.push('/')}>Create Push</Button>}
      {state.loading ? <Loading size="50px"/> :
