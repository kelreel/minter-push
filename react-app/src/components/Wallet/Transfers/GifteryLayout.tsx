...
const GifteryLayout: React.FC = observer(() => {
  ...
  const [state, setState] = useState<{
    ...
    currentItem: GifteryProduct | null;
  }>({
    modal: false,
    products: null,
    currentItem: null
    ...
    <>
      ...
      {state.products?.food && (
        <>
          ...
          <div className="shops animated fadeInUp">{getCards("food")}</div>
        </>
      )}
      ...
      {state.products?.internet && (
        <>
          <div className="title" style={{ color: pStore.categoryTitleColor }}>
            {pStore.showCategoryTitle && <>{t("giftery.internet")}</>}
          </div>

          <div className="shops">{getCards("internet")}</div>
        </>
      )}
      ...
      {state.products?.shops && (
        <>
          ...
          <div className="shops">{getCards("shops")}</div>
        </>
      )}
      ...
      {state.products?.games && (
        <>
          ...
          <div className="shops">{getCards("games")}</div>
        </>
      )}
      ...
      {state.currentItem && (
        <Giftery
          visible={state.modal}
          brief={state.currentItem.brief}
          digital_acceptance={state.currentItem?.brief}
          disclaimer={state.currentItem?.disclaimer}
          face_max={state.currentItem?.face_max}
          face_min={state.currentItem?.face_min}
          face_step={state.currentItem?.face_step}
