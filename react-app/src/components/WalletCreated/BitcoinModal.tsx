...
export const BitcoinModal: React.FC<{ visible: boolean, address: string }> = ({visible, address}) => {
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      setState({...state, success: true, bitcoinAddress: res.data.data.address, loading: false})
    } catch (error) {
  ...
  return <Modal
    ...
    footer={
      <>
        {!state.success && <><Button key="back" onClick={handleCancel}>
  ...
  >
    {state.success ? <div className="bitcoin-refill-success">
      <p style={{textAlign: 'center'}}>Bitcoin will be converted to BIP after 2 confirmations</p>
      <div className="address">
        <p onClick={copyAddress} style={{cursor: 'pointer'}}>{state.bitcoinAddress}</p>
      </div>
      <QRCodeCanvas value={address} onClick={copyAddress} size={200}/>
    </div> : <div className="bitcoin-refill">
      <div className="field">
        <label>E-Mail</label>
        <Input autoFocus onPressEnter={handleOk} placeholder="hello@mail.com" value={state.email} onChange={(e) => {
