...
export const mailRegExp = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
  ...
  ({
    ...
    return (
      <Modal
        ...
        centered={isMobile}
        bodyStyle={{
          backgroundImage: `linear-gradient(rgba(255,255,255,.97), rgba(255,255,255,.97)), url(https://${image_url})`
        }}
        footer={
          !state.error &&
          !state.success && (
            <>
              ...
              <Button
                disabled={
                  !state.face || !state.coin || !mailRegExp.test(state.email)
                }
                key="submit"
                type="primary"
        ...
        <>
            ...
            <div className="send-form">
              ...
              <Collapse bordered={false} accordion>
                <Panel header={t("giftery.modal.disclaimer")} key="2">
                  <div className="disclaimer">{parse(disclaimer)}</div>
                </Panel>
              </Collapse>
                ...
                <div className="price">
                  {state.coin === "BIP" ? (
                    <>
                      ...
                      <p>
                        {Math.round(
                          (+state.face / store.rubCourse / store.bipPrice) * 100
                        ) / 100}{" "}
                        {state.coin}
                      </p>
                    </>
          ...
          {state.success && (
            <Result
              ...
              extra={
                <Button type="primary" onClick={() => getPDF(state.order!)}>
